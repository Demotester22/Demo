...
$sql="select * from movie where movie_id='$movie' and city_id='$city'" ;
$sqlresult=mysql_query($sql);
$row = mysql_fetch_array($sqlresult);
$moviename=$row['movie_name'];

$sql="select * from movie where date='$date' and movie_id='$movie' and city_id='$city'";
$sqlresult=mysql_query($sql);
$row = mysql_fetch_array($sqlresult);
$date2=$row['date'];
?>
  ...
  <tr>
    <td>City</td>
    <td><input name="city" type="text" id="city" readonly="true" style="background-color:#000; color:#FFF" value="<?php echo $cityname ;  ?>"/></td>
  </tr>
  <tr>
    <td>Movie</td>
    <td><input name="movie" type="text" id="movie" readonly="true" style="background-color:#000; color:#FFF" value="<?php echo $moviename ; ?>" />	</td>
  </tr>
  <tr>
    <td>Date</td>
    <td><input name="date" type="text" id="date" readonly="true" style="background-color:#000; color:#FFF" value="<?php echo $date2 ; ?>" /></td>
  </tr>
</table>
  <?php
  	echo "<br><br>";
  	$sql = "Select theatre_id,showtiming,price from movie where movie_id='$movie' and city_id='$city' and date='$date'";
	$result = mysql_query($sql);
	echo "<table>";
	echo "<tr>
	...
	while($row = mysql_fetch_array($result))
	{
		  echo "<form name=\"form1\" action=\"book.php\" method=\"post\">";
			...
			<td><input name=\"price\" type=\"text\" id=\"stime\" readonly=\"true\" style=\"background-color:#000; color:#FFF\" value='$price'/></td>
